...
import java.awt.*;
import java.awt.event.*; // ✅ KeyEvent, etc
...
public class AddEditResidentDialog {
    ...
    public AddEditResidentDialog(JFrame parent, Resident existingResident, List<Resident> residents) {
        ...
        this.householdMembers = existingResident != null ? new ArrayList<>(existingResident.getHouseholdMembers())
                : new ArrayList<>();

        dialog = new JDialog(parent, existingResident == null ? "Add New Resident (Household Head)" : "Edit Resident",
                true);
        ...
        JLabel titleLabel = new JLabel(
                existingResident == null ? "Add New Resident - Household Head" : "Edit Resident");
        ...
        if (existingResident != null)
            firstNameField.setText(existingResident.getFirstName());
        ...
        if (existingResident != null)
            lastNameField.setText(existingResident.getLastName());
        ...
        if (existingResident != null)
            qualifierField.setText(
                    existingResident.getSuffix() != null ? existingResident.getSuffix() : "");
        ...
        if (existingResident != null)
            ageField.setText(String.valueOf(existingResident.getAge()));
        ...
        if (existingResident != null)
            birthdayField.setText(
                    existingResident.getDateofBirth() != null
                            ? existingResident.getDateofBirth().toString()
                            : "");
        ...
        if (existingResident != null)
            sexComboBox.setSelectedItem(existingResident.getSex());
        ...
        if (existingResident != null)
            medicalConditionField.setText(existingResident.getMedicalCondition());
        ...
        if (existingResident != null) {
            ...
            if (income < 20000)
                incomeComboBox.setSelectedItem("Less than ₱20,000");
            else if (income <= 40000)
                incomeComboBox.setSelectedItem("₱20,000 - ₱40,000");
            else if (income <= 60000)
                incomeComboBox.setSelectedItem("₱40,001 - ₱60,000");
            else if (income <= 80000)
                incomeComboBox.setSelectedItem("₱60,001 - ₱80,000");
            else if (income <= 100000)
                incomeComboBox.setSelectedItem("₱80,001 - ₱100,000");
            else if (income > 100000)
                incomeComboBox.setSelectedItem("More than ₱100,000");
            else
        ...
        if (existingResident != null)
            motherTongueField.setText(existingResident.getMotherTongue());
        ...
        if (existingResident != null)
            occupationField.setText(existingResident.getOccupation());
        ...
        if (existingResident != null)
            addressField.setText(existingResident.getAddress());
        ...
        if (existingResident != null)
            contactField.setText(existingResident.getContactNumber());
        ...
        StyledButton saveButton = new StyledButton(
                "Save Resident",
                BarangayColors.BUTTON_BLACK,
                Color.WHITE,
                BarangayColors.BUTTON_HOVER,
                BarangayColors.BUTTON_ACTIVE);
        saveButton.setPreferredSize(new Dimension(150, 35));
        saveButton.addActionListener(e -> saveResident());

        StyledButton cancelButton = new StyledButton(
                "Cancel",
                BarangayColors.BUTTON_BLACK,
                Color.WHITE,
                BarangayColors.BUTTON_HOVER,
    ...
    private void showAddMemberDialog() {
        ...
        addButton.addActionListener(e -> {
            ...
            String birthdayStr = birthdayField.getText().trim(); // YYYY-MM-DD
            String civilStatus = civilStatusField.getText().trim();

            if (lastName.isEmpty() || firstName.isEmpty() || birthdayStr.isEmpty() || civilStatus.isEmpty()) {
            ...
            LocalDate dob;
            try {
                dob = LocalDate.parse(birthdayStr); // expects YYYY-MM-DD
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(memberDialog,
            ...
            // ✅ create member using your HouseholdMember constructor
            ...
            memberTableModel.addRow(new Object[] {
                    ...
                    member.getFirstName(),
                    member.getQualifier(),
                    member.getAge(), // computed from dob
                    member.getDateofBirth() != null ? member.getDateofBirth().toString() : "",
                    member.getCivilStatus()
            });
    ...
    private void saveResident() {
        try {
            ...
            if (incomeSelection != null && !incomeSelection.equals("Select income level")) {
                if (incomeSelection.equals("Less than ₱20,000"))
                    incomeBracket = 15000;
                else if (incomeSelection.equals("₱20,000 - ₱40,000"))
                    incomeBracket = 30000;
                else if (incomeSelection.equals("₱40,001 - ₱60,000"))
                    incomeBracket = 50000;
                else if (incomeSelection.equals("₱60,001 - ₱80,000"))
                    incomeBracket = 70000;
                else if (incomeSelection.equals("₱80,001 - ₱100,000"))
                    incomeBracket = 90000;
                else if (incomeSelection.equals("More than ₱100,000"))
                    incomeBracket = 120000;
            }
            ...
            if (resident == null) {
                ...
                Resident r = new Resident(
                        id,
                        firstName,
                        mInitialField.getText().trim(),
                        lastName,
                        qualifier,
                        LocalDate.parse(birthdayField.getText().trim()),
                        sex,
                        medicalConditionField.getText().trim(),
                        incomeBracket,
                ...
                resident.setMiddleName(mInitialField.getText().trim()); // if your Resident has this
                resident.setSuffix(qualifier); // if qualifier == suffix
                resident.setDateofBirth(LocalDate.parse(birthdayField.getText().trim()));
                resident.setSex(sex);
